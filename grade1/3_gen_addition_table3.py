...
# 生成随机的加法和减法题目，结果不显示
def generate_random_addition_subtraction(num_problems=20, max_numb=20, generate_subtraction=False):
    problems = []

    # greater_than_max_numb_ratio 是根据 max_numb 计算的比例，改变为 max_numb / 2
    greater_than_max_numb_ratio = max_numb / 2
    count_gt_max_numb = int(num_problems * greater_than_max_numb_ratio)  # 大于 max_numb 的题目数
    count_le_max_numb = num_problems - count_gt_max_numb  # 小于等于 max_numb 的题目数

    # 如果生成减法题目
    if generate_subtraction:
        # 生成减法题目（从 max_numb 开始减）
        while len(problems) < num_problems:
            a = random.randint(max_numb, max_numb * 2)  # 从 max_numb 到 max_numb * 2 之间随机选择
            b = random.randint(1, a)  # 保证结果为正数
            problems.append(f"{a}-{b}=")

    # 如果只生成加法题目
    else:
        # 生成大于 max_numb 且小于等于 max_numb * 2 的加法题目
        while len(problems) < count_gt_max_numb:
            a = random.randint(1, max_numb)  # 生成第一个加数，a 可以是 1 到 max_numb 的任意数
            b = random.randint(1, max_numb)  # 生成第二个加数，b 也可以是 1 到 max_numb 的任意数
            if a + b >= max_numb:  # 保证加法结果大于等于 max_numb
                problems.append(f"{a}+{b}=")

        # 生成小于等于 max_numb 的加法题目
        while len(problems) < num_problems:
            a = random.randint(1, max_numb)  # 生成加数 a 小于等于 max_numb
            b = random.randint(1, max_numb)  # 生成加数 b 小于等于 max_numb
            if a + b <= max_numb:  # 保证结果小于等于 max_numb
                problems.append(f"{a}+{b}=")

    random.shuffle(problems)  # 随机打乱题目顺序
    problems = problems[:num_problems]  # 确保最终返回的题目数量为 num_problems
    return problems
...
def main():
    ...
    max_numb = 20  # 最大值（加法和减法的最大数值）
    generate_subtraction = False  # 是否生成减法题目
    line_spacing = 0.05  # 控制行间距
    problems = generate_random_addition_subtraction(num_problems, max_numb, generate_subtraction)
