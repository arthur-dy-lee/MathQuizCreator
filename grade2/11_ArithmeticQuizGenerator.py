...
1、不允许负数
2、出题的形式： A+b=c, 任何位置可下划线填空，比如 22 - __ =  6
3、每题前都有标号，顺序递增。
4、参数：1. 可以控制一次出多少道题，默认999道题。 2. 每行多少道题，默认是4个。 3. 每个题结束后，有多少个空格，默认是1个。
4. 加法、减法、乘法、除法，都可以做控制，比如只出加法、减法的题；只出 加减乘法的题。5、还有一个参数控制，如果有乘法，是否是仅限 9*9乘法表的题。
6. 控制出题的数值范围，默认 0~109， 无论是计算过程还是结果，都不超过109这个默认最大值。范围是可以调整的。7. 答案在最后，每个答案也是带标号的。
8. 要填的数值是下划线，默认是2个下划线。 9. 帮我想一个类名和main运行。
10. 参数：若带有加减法，每100题，它的进位比例可参数化，比如占比50%是它的默认值。
11. 进位乘法参数。若带有乘法，multiplication_table_only: bool = False ，则可进位比例默认60%，这时在的60%是所有的乘法中的占比。
第1个50%，也是所有加减法中进位的比例。乘法的进位，是指9*9以外的乘法。当然，这个结果也是在min~max值之间的。
12. 加法乘法比例参数：如果同时存在加减乘法、或加减乘除法，那么加减法是一组，默认比便是80%，乘除法是一组，则为剩余的 20%了。
13、目前的计算一般情况都是100以内的计算，所以最好能做对齐。因为有的是个位数有的是十位数，所以有时候对不齐比较难看一些。
"""
...
class QuizConfig:
    """题目配置类"""
    # 基础参数
    total_questions: int = 1000             # 总题数
    questions_per_row: int = 4              # 每行题数
    spaces_after_question: int = 2          # 题目间空格数
    blank_underscores: int = 2              # 下划线数量
    num_align_width: int = 3                # 数字对齐宽度（默认3位数对齐）
...
class ArithmeticQuizGenerator:
    ...
    def format_questions(self) -> str:
        ...
        for i, q in enumerate(self.questions):
            is_first_in_row = (i % self.config.questions_per_row == 0)
